...
function getLoadedRealCostPerHour(profile) {
  return num(
    profile?.realCostPerHour,
    profile?.effectiveCost,
    profile?.results?.effectiveCost,
    profile?.results?.realCostPerHour,
    profile?.results?.commercial?.realCostPerHour,
    ...
    profile?.loaded?.effectiveCost,
    profile?.loaded?.realCostPerHour
  );
}

function getProductiveHours(profile) {
  const directProductiveHours = num(
    profile?.productiveHoursPerYear,
    profile?.results?.productiveHoursPerYear,
    profile?.results?.loaded?.productiveHoursPerYear,
    profile?.loaded?.productiveHoursPerYear,
    profile?.inputs?.productiveHoursPerYear,
    profile?.form?.productiveHoursPerYear,
    profile?.results?.inputs?.productiveHoursPerYear
  );

  if (directProductiveHours > 0) {
    return directProductiveHours;
  }
  ...
  const calculatedProductiveHours = paidHoursPerYear * (efficiencyPct / 100);

  return calculatedProductiveHours > 0 ? calculatedProductiveHours : 0;
}
...
function getProductivityPct(profile, productiveHours = 0, paidHours = 0) {
  ...
  if (paidHours > 0 && productiveHours > 0) {
    return (productiveHours / paidHours) * 100;
  }

  return 0;
}

function getTargetRate(profile) {
  return num(
    profile?.targetRate,
    profile?.results?.targetRate,
    profile?.results?.profit?.targetRate,
    profile?.results?.commercial?.targetRate,
    profile?.commercial?.targetRate,
    profile?.results?.loaded?.targetRate,
    profile?.loaded?.targetRate
  );
}
...
export function loadCommercialPortfolioData() {
  if (typeof window !== "undefined") {
    ...
    for (let i = 0; i < window.localStorage.length; i += 1) {
      ...
      console.log("LOCAL STORAGE VALUE:", rawValue);
    }
  }
...
export function buildCommercialPortfolio(
  ...
  console.log("=== COMMERCIAL PORTFOLIO BUILD START ===");
  console.log("LABOUR PROFILES LOADED:", labourProfiles);
  console.log("OVERHEAD PROFILES LOADED:", overheadProfiles);
  ...
  const rawRows = labourProfiles.map((labourProfile, index) => {
    ...
    const labourId = getProfileId(labourProfile);
    const labourName = getLabourName(labourProfile);
    const labourProfileName = getLabourProfileName(labourProfile);

    console.log("----- COMMERCIAL MATCH DEBUG -----");
    console.log("LABOUR PROFILE RAW:", labourProfile);
    console.log("LABOUR ID:", labourId);
    console.log("LABOUR NAME:", labourName);
    console.log("LABOUR PROFILE NAME:", labourProfileName);
    console.log("MATCHED OVERHEAD:", overheadProfile);
    console.log(
      "ALL OVERHEAD CANDIDATES:",
      overheadProfiles.map((item) => ({
        raw: item,
        overheadName: getOverheadName(item),
        linkedName: getLinkedLabourProfileName(item),
        linkedId: getLinkedLabourProfileId(item),
        annualOverhead: getOverheadAnnual(item),
      }))
    );
    console.log("----- END COMMERCIAL MATCH DEBUG -----");

    const productiveHours = getProductiveHours(labourProfile);
    ...
    const baseRealCostPerHour = getLoadedRealCostPerHour(labourProfile);
    const annualOverhead = overheadProfile ? getOverheadAnnual(overheadProfile) : 0;
    const overheadPerHour =
      productiveHours > 0 ? annualOverhead / productiveHours : 0;

    const realCostPerHour = baseRealCostPerHour + overheadPerHour;
    const chargeoutRate = getChargeoutRate(labourProfile);
    const targetMarginPct = getTargetMarginPct(labourProfile);
    const targetRate = getTargetRate(labourProfile);
    const profitPerHour = chargeoutRate - realCostPerHour;

    const marginPct =
      chargeoutRate > 0 ? (profitPerHour / chargeoutRate) * 100 : 0;

    const annualRevenue = chargeoutRate * productiveHours;
    const annualRealCost = realCostPerHour * productiveHours;
    const annualProfit = profitPerHour * productiveHours;
  ...
  const totalChargeoutPerHour = rows.reduce(
    (sum, row) => sum + num(row.chargeoutRate),
    0
  );
  const totalRealCostPerHour = rows.reduce(
    (sum, row) => sum + num(row.realCostPerHour),
    0
  );
  const totalProfitPerHour = rows.reduce(
    (sum, row) => sum + num(row.profitPerHour),
    0
  );
  ...
  const totalProductiveHours = rows.reduce(
    (sum, row) => sum + num(row.productiveHours),
    0
  );
  ...
  const portfolioMarginPct =
    totalChargeoutPerHour > 0
      ? (totalProfitPerHour / totalChargeoutPerHour) * 100
      : 0;

  const portfolioAnnualMarginPct =
    totalAnnualRevenue > 0
      ? (totalAnnualProfit / totalAnnualRevenue) * 100
      : 0;

  const blendedChargeoutRate =
    profileCount > 0 ? totalChargeoutPerHour / profileCount : 0;
  const blendedRealCostRate =
    profileCount > 0 ? totalRealCostPerHour / profileCount : 0;
  const blendedProfitPerHour =
    profileCount > 0 ? totalProfitPerHour / profileCount : 0;
  const avgTargetMarginPct =
    profileCount > 0
      ? rows.reduce((sum, row) => sum + num(row.targetMarginPct), 0) / profileCount
      : 0;

  const totalsPeriodValues = buildPeriodValues({
    hourlyRevenue:
      totalProductiveHours > 0 ? totalAnnualRevenue / totalProductiveHours : 0,
    hourlyRealCost:
      totalProductiveHours > 0 ? totalAnnualRealCost / totalProductiveHours : 0,
    hourlyOverhead:
      totalProductiveHours > 0 ? totalAnnualOverhead / totalProductiveHours : 0,
    hourlyProfit:
      totalProductiveHours > 0 ? totalAnnualProfit / totalProductiveHours : 0,
  ...
  console.log("COMMERCIAL ROWS BUILT:", rows);
  console.log("=== COMMERCIAL PORTFOLIO BUILD END ===");
