// lib/calculations/commercialSummaryHelpers.js
...
export function calculateCommercialSummary({
  ...
  const recoveredLabourBeforeOverheads = round2(
    results?.loaded?.effectiveRecoveredCostPerHourBeforeOverheads || 0
  );

  const derivedOverheadPerHour = round2(
    (results?.loaded?.effectiveRecoveredCostPerHour || 0) -
      (results?.loaded?.effectiveRecoveredCostPerHourBeforeOverheads || 0)
  );

  const overheadPerHour = round2(
    results?.loaded?.overheadPerHour ||
      derivedOverheadPerHour ||
  ...
  const targetRate = round2(results?.loaded?.targetRate || 0);
  const rateGapToTarget = round2(results?.loaded?.rateGapToTarget || 0);

  const chargeoutRate = round2(
    results?.profit?.chargeoutRate || results?.inputs?.chargeoutRate || 0
  );

  const profitPerHour = round2(results?.profit?.profitPerHour || 0);
  const marginPct = round2(results?.profit?.marginPct || 0);
  const markupPct = round2(results?.profit?.markupPct || 0);
